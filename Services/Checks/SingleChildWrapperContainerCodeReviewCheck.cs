...
public sealed class SingleChildWrapperContainerCodeReviewCheck : CodeReviewCheckBase
{
    public override string RuleId => CodeReviewRuleIds.SingleChildWrapperContainer;

    public override string DisplayName => "Single-child wrapper containers";
        ...
        foreach (var file in context.Files)
        {
            ...
            foreach (var container in document.Descendants().Where(MarkupContainerCodeReviewUtilities.IsTargetMultiChildContainer))
            {
                ...
                if (visualChildren.Count != 1)
                    continue;
                if (MarkupContainerCodeReviewUtilities.IsNestedSamePanelWithoutEffect(container, visualChildren[0]))
                    continue;
                ...
                AddFinding(
                    ...
                    $"`{container.Name.LocalName}` contains only one child. Consider removing this wrapper container.");
            }
        }
    }
}
